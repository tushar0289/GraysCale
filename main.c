...
int main(int argc, char *argv[]){
    ...
    selection_sort(arr, idx, argc - 1);
    printf("File size order\n");
    for(int i = 0; i < argc - 1; i++)
        printf("File Name: %s\nFile Size: %d\n\n", argv[idx[i]], arr[i]);

    for(int i = 0; i < argc - 1; i++){
        FILE *fp = fopen(argv[idx[i]], "rb");
        ...
        */
        ...
        if(dibHeader.bitCount == 32){
            printf("File %s is 32bits!\n Cannot convert 32 bits file.", argv[idx[i]]);
            fclose(fp);
            continue;
        }
        ...
        uint8_t file_name_size = strlen(argv[idx[i]]);

        char file_name[100];
        strncpy(file_name, argv[idx[i]], file_name_size - 4);
        file_name[file_name_size - 4] = '\0';

        char text_file_name[file_name_size];
        strcpy(text_file_name, file_name);
        strcat(text_file_name, ".txt");
        
        char rgb_file_name[100];
        sprintf(rgb_file_name, "%s_%s.%s", file_name, "rgb", "txt");

        FILE *write = fopen(text_file_name, "w");
        if(write == NULL){
            printf("Error Opening \"%s\" File\n", text_file_name);
            return 1;
        }

        printf("Pixel Data in Luminance value\n\n");

        char luminance_name[100];
        sprintf(luminance_name, "%s_%s.%s", file_name, "luminance", "txt");


        FILE *lumi = fopen(luminance_name, "w");
        if(lumi == NULL){
            printf("Error Opening \"%s\" File\n", luminance_name);
            return 1;
        }
        ...
        char grayscale_name[100];
        sprintf(grayscale_name, "%s_%s.%s", file_name, "grayscale", "bmp");

        FILE *fgray = fopen(grayscale_name, "wb");
        if(fgray == NULL){
            printf("Error Opening \"%s\" File", grayscale_name);
            return 1;
        }
        ...
        // dither grayscale
        char dither_name[100];
        sprintf(dither_name, "%s_%s.%s", file_name, "dither", "bmp");

        FILE *fdither = fopen(dither_name, "wb");
        ...
        char ascii_name[100];
        sprintf(ascii_name, "%s_%s.%s", file_name, "ascii", "txt");

        FILE *fascii = fopen(ascii_name, "wb");
